...
def mock_env_development():
    ...
    with patch.dict(
        os.environ,
        {"ENVIRONMENT": "development", "ALLOWED_ORIGINS": "https://custom-dev.com"},
        clear=True,
    ):
        yield
...
def test_app():
    ...
    async def test_credentials(request: Request):
        ...
        return {"has_auth": bool(auth_header), "has_cookie": bool(cookie_header)}
...
class TestCORSPreflightRequests:
    ...
    def test_preflight_blocked_origin(self, mock_env_production, test_app):
        ...
        response = client.options(
            "/test", headers={"Origin": "https://evil.com", "Access-Control-Request-Method": "POST"}
        )
...
class TestCORSIntegration:
    ...
    async def test_api_endpoints_cors(self, mock_env_production):
        ...
        with patch.dict(
            ...
            response = client.options(
                "/api/v1/generate",
                headers={"Origin": "https://hacker.com", "Access-Control-Request-Method": "POST"},
            )
