...
class TestCORSWithAuthentication:
    ...
    def test_authenticated_endpoint_allowed_origin(
        self, test_app_with_auth, valid_token, auth_middleware
    ):
    ...
    def test_authenticated_endpoint_blocked_origin(
        self, test_app_with_auth, valid_token, auth_middleware
    ):
        ...
        with patch("music_gen.api.middleware.auth.auth_middleware", auth_middleware):
            ...
            response = client.get(
                "/authenticated",
                headers={"Origin": "https://evil.com", "Authorization": f"Bearer {valid_token}"},
            )
    ...
    def test_authenticated_endpoint_no_token(self, test_app_with_auth):
        ...
        response = client.get("/authenticated", headers={"Origin": "http://localhost:3000"})

        assert response.status_code == 401
        # Should still include CORS headers for allowed origin
        assert response.headers.get("access-control-allow-origin") == "http://localhost:3000"

    def test_admin_endpoint_cors(
        self, test_app_with_auth, admin_token, valid_token, auth_middleware
    ):
...
class TestCORSWithDifferentEnvironments:
    ...
    def test_staging_cors_with_auth(self, auth_middleware, valid_token):
        ...
        with patch.dict(
            os.environ,
            {"ENVIRONMENT": "staging", "STAGING_DEV_ORIGINS": "http://localhost:3000"},
            clear=True,
        ):
            app = create_app()
            client = TestClient(app)
...
class TestOptionalAuth:
    ...
    def test_optional_auth_no_token(self, test_app_with_auth):
        ...
        response = client.get("/optional-auth", headers={"Origin": "http://localhost:3000"})
    ...
    def test_generation_endpoint_cors_auth(self, auth_middleware, valid_token):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
            ...
            with patch("music_gen.api.middleware.auth.auth_middleware", auth_middleware):
                ...
                assert (
                    "access-control-allow-origin" in response.headers or response.status_code >= 400
                )
