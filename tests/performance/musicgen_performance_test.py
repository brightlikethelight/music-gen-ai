...
class PerformanceBenchmark:
    ...
    def measure_performance(self, model_name, prompt, duration):
        ...
        try:
            ...
            processor = AutoProcessor.from_pretrained(
                f"facebook/musicgen-{model_name}", cache_dir="cache"
            )
            ...
            with torch.no_grad():
                if device == "cuda":
                    ...
                    audio_values = model.generate(
                        **inputs, max_new_tokens=tokens, do_sample=True, temperature=1.0, top_p=0.9
                    )
            ...
            print(
                f"✓ Generated in {generation_time:.1f}s ({metrics['realtime_factor']:.1f}x realtime)"
            )
    ...
    def _generate_report(self, system_info):
        ...
        for r in successful:
            ...
            print(
                f"  {r['model']}-{r['test_name']}: {speed:.1f}x realtime, "
