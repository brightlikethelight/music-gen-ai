...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("musicgen_test.log"), logging.StreamHandler(sys.stdout)],
)
...
class MusicGenTester:
    ...
    def get_memory_usage(self):
        ...
        return {
            ...
            "gpu_memory_gb": (
                torch.cuda.memory_allocated() / 1024**3 if self.device == "cuda" else 0
            ),
        }
        ...
        try:
            ...
            def progress_callback(current_tokens):
                ...
                if new_stages > stages_complete:
                    ...
                    logger.info(
                        f"Progress: {'█' * stages_complete}{'░' * (total_stages - stages_complete)} "
    ...
    def validate_audio(self, audio: np.ndarray, expected_duration: float) -> Dict:
        ...
        if abs(actual_duration - expected_duration) > 2.0:
            validation["issues"].append(
                f"Duration mismatch: expected {expected_duration}s, got {actual_duration:.1f}s"
            )
        ...
        logger.info(
            f"Audio stats: Duration={actual_duration:.1f}s, RMS={validation['stats']['rms']:.3f}, "
    ...
    def generate_report(self, results: List[Dict]):
        ...
        # Save final report
        report_file = (
            self.output_dir / f"test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
