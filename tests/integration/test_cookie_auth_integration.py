...
class MockFrontendClient:
    ...
    def get(self, path: str, **kwargs):
        ...
        response = self.client.get(path, cookies=self.cookies, **kwargs)
    ...
    def post(self, path: str, json_data=None, headers=None, **kwargs):
        ...
        response = self.client.post(
            path, json=json_data, headers=headers, cookies=self.cookies, **kwargs
        )
        self._update_cookies(response)
        return response
    ...
    def delete(self, path: str, headers=None, **kwargs):
        ...
        headers = self._prepare_headers(headers)
        response = self.client.delete(path, headers=headers, cookies=self.cookies, **kwargs)
        self._update_cookies(response)
        return response
...
class TestCookieAuthIntegration:
    ...
    def test_login_with_remember_me(self):
        ...
        login_data = {"email": "user@example.com", "password": "password123", "remember_me": True}
    ...
    def test_concurrent_sessions(self):
        ...
        for client in [client1, client2]:
            ...
            response = client.post(
                "/api/auth/login",
                json_data={"email": "user@example.com", "password": "password123"},
            )
            assert response.status_code == 200
    ...
    def test_api_endpoint_protection(self):
        ...
        self.client.get("/api/auth/csrf-token")
        response = self.client.post(
            "/api/auth/login", json_data={"email": "user@example.com", "password": "password123"}
        )
        assert response.status_code == 200
    ...
    def test_cross_domain_cookie_handling(self):
        ...
        headers = {"Origin": "https://app.musicgen.ai", "Referer": "https://app.musicgen.ai/"}
