...
class TestClient:
    ...
    async def generate_music(self, prompt: str, duration: float = 10.0) -> Dict:
        ...
        response = await self.client.post(f"{self.base_url}/generate", json=data, headers=headers)
        response.raise_for_status()
    ...
    async def get_job_status(self, job_id: str) -> Dict:
        ...
        response = await self.client.get(f"{self.base_url}/generate/job/{job_id}", headers=headers)
        response.raise_for_status()
    ...
    async def create_playlist(self, name: str) -> Dict:
        ...
        response = await self.client.post(f"{self.base_url}/playlists", json=data, headers=headers)
        response.raise_for_status()
