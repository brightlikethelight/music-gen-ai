...
class TestBasicAugmentations:
    ...
    def test_add_noise(self, sample_audio):
        ...
        for noise_type in noise_types:
            ...
            assert not torch.allclose(result, waveform), f"Noise type {noise_type} should change the signal"
            ...
            assert 0.8 < ratio < 1.5, f"RMS ratio {ratio} is too extreme for noise type {noise_type}"
    ...
    def test_frequency_masking(self, sample_audio):
        ...
        # Result should be different from original
        assert not torch.allclose(result, waveform, atol=1e-6), "Frequency masking should change the signal"
    ...
    def test_distortion(self, sample_audio):
        ...
        # Result should be different
        assert not torch.allclose(result, waveform, atol=1e-6), "Distortion should change the signal"
...
class TestPolymixAugmentation:
    ...
    def test_polymix_with_samples(self, mix_samples):
        ...
        assert not torch.allclose(result, original), "Polymix should change the signal when mix samples are available"
