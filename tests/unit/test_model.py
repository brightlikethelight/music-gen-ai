...
class TestModelModel:
    ...
    def test_multiheadattention_forward(self, device, test_config):
        ...
        hidden_states = torch.randn(batch_size, seq_len, test_config.transformer.hidden_size).to(device)
    ...
    def test_feedforward_forward(self, device, test_config):
        ...
        batch_size, seq_len = 2, 50
        input_tensor = torch.randn(batch_size, seq_len, test_config.transformer.hidden_size).to(device)
    ...
    def test_musicgentransformer_forward(self, device, test_config):
        ...
        batch_size, seq_len = 2, 50
        input_ids = torch.randint(0, test_config.transformer.vocab_size, (batch_size, seq_len)).to(device)
