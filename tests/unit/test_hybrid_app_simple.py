...
with patch.dict('sys.modules', {
    'musicgen.infrastructure.config.config': MagicMock(config=MagicMock(
        OUTPUT_DIR="./outputs",
        LOG_LEVEL="INFO",
        CORS_ORIGINS=["*"],
    ...
    from musicgen.api.rest.hybrid_app import app, GenerationRequest, GenerationResponse, JobStatus
...
class TestHybridAppEndpoints:
    ...
    def test_job_status_endpoint(self, client):
        ...
        # Create a job first
        with patch('musicgen.api.rest.hybrid_app._jobs', {"test-123": JobStatus(
            job_id="test-123",
            status="completed",
            progress=1.0,
            message="Done"
