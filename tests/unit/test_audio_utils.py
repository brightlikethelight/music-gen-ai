...
class TestAudioProcessing:
    ...
    def test_apply_fade(self, sample_audio):
        ...
        faded = apply_fade(audio, sample_rate, fade_duration, fade_duration)
        
        # Check fade in
        fade_samples = int(fade_duration * sample_rate)
        assert faded[0, 0] == 0.0  # Starts at zero
        assert faded[0, fade_samples-1] < audio[0, fade_samples-1]  # Faded in
        
        # Check fade out
        assert faded[0, -1] == 0.0  # Ends at zero
        assert faded[0, -fade_samples] < audio[0, -fade_samples]  # Faded out
        
        # Middle should be unchanged
        mid_start = fade_samples + 100
        mid_end = -fade_samples - 100
        assert torch.allclose(
            faded[0, mid_start:mid_end], 
            audio[0, mid_start:mid_end], 
            atol=1e-6
        )
    ...
    def test_trim_silence(self):
        ...
        audio = torch.cat([
            ...
            torch.sin(2 * np.pi * 440 * torch.linspace(0, audio_duration, audio_samples)).unsqueeze(0),  # Audio
        ...
        # Should contain the audio part
        assert trimmed.shape[1] >= audio_samples
    ...
    def test_concatenate_audio(self):
        ...
        concatenated = concatenate_audio([audio1, audio2], crossfade_duration=0.0, sample_rate=sample_rate)
...
class TestAudioIO:
    ...
    @patch('music_gen.utils.audio.torchaudio.load')
    @patch('music_gen.utils.audio.torchaudio.save')
    def test_convert_audio_format_mock(self, mock_save, mock_load):
        ...
        mock_load.assert_called_once_with("/fake/input.mp3", frame_offset=0, num_frames=-1)
        mock_save.assert_called_once()
