...
class VocalGenPipeline:
    ...
    def generate_with_vocals(
        ...
        try:
            ...
            vocal_audio, vocal_sr = self.vocal_synthesizer.generate_singing(
                lyrics=lyrics_text, style=vocal_style
            )
            ...
            if hasattr(self.melody_synthesizer, "apply_melody_contour"):
                vocal_audio = self.melody_synthesizer.apply_melody_contour(
                    vocal_audio,
                    vocal_sr,
                    melody_pattern=self._get_melody_pattern(vocal_style),
                )
    ...
    def generate_instrumental_only(
        self, prompt: str, duration: float = 30.0, enhance: bool = True, **kwargs
    ) -> Tuple[np.ndarray, int]:
        """
        ...
        # Generate
        if duration > 30:
            audio, sr = self.music_generator.generate_extended(
    ...
    def remix_with_vocals(
        ...
        # Generate vocals
        vocal_audio, vocal_sr = self.vocal_synthesizer.generate_singing(
            lyrics=lyrics, style=vocal_style
        )
    ...
    def _detect_style_from_prompt(self, prompt: str) -> str:
        ...
        if any(word in prompt_lower for word in ["jazz", "swing", "bebop"]):
            ...
            word in prompt_lower for word in ["electronic", "techno", "house", "edm"]
        ):
            return "electronic"
        elif any(
            word in prompt_lower for word in ["classical", "orchestra", "symphony"]
        ):
            return "classical"
        else:
            return "pop"  # Default
    ...
    def _get_mix_parameters(
        self, mix_style: str, vocal_style: str
    ) -> Tuple[float, float]:
