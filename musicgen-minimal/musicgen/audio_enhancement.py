...
try:
    ...
    from pedalboard import (
        Pedalboard, Compressor, Gain, Limiter, Reverb,
...
class AudioEnhancer:
    ...
    def reduce_artifacts(
        ...
        audio: np.ndarray, 
        sample_rate: int,
        noise_profile_duration: float = 0.1
    ) -> np.ndarray:
        """
    ...
    def sidechain_compress(
        ...
        for i in range(1, len(envelope)):
            if envelope[i] > smoothed[i-1]:
                # Attack
                smoothed[i] = smoothed[i-1] + (envelope[i] - smoothed[i-1]) / attack_samples
            else:
                # Release
                smoothed[i] = smoothed[i-1] + (envelope[i] - smoothed[i-1]) / release_samples
