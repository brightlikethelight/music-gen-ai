...
class PromptEngineer:
    ...
    def _load_instruments(self) -> Dict[str, List[str]]:
        ...
        return {
            "strings": ["violin", "cello", "guitar", "bass", "harp", "acoustic guitar", "electric guitar"],
            "wind": ["saxophone", "trumpet", "flute", "clarinet", "harmonica"],
            "keys": ["piano", "synthesizer", "organ", "electric piano", "harpsichord"],
            "percussion": ["drums", "percussion", "timpani", "tambourine", "shaker", "808"],
            "electronic": ["synth lead", "synth bass", "pad", "arpeggiator", "808 bass", "samples", "vinyl"]
        }
    
    def _load_modifiers(self) -> Dict[str, List[str]]:
        """Effective modifier words grouped by category."""
        return {
            "energy": ["energetic", "calm", "intense", "relaxed", "aggressive", "peaceful"],
            "mood": ["happy", "sad", "melancholic", "uplifting", "dark", "bright", "mysterious"],
            "tempo": ["slow", "moderate", "fast", "upbeat", "downtempo", "mid-tempo"],
            "texture": ["smooth", "rough", "ethereal", "dense", "sparse", "rich", "minimal"]
        }
    
    def _load_avoid_words(self) -> List[str]:
        """Words that often lead to poor results with MusicGen."""
        return [
            "vocal", "vocals", "singing", "voice", "lyrics",  # MusicGen is instrumental only
            "epic", "cinematic", "hollywood",  # Too vague, often disappoints
            "masterpiece", "professional", "high quality",  # Quality descriptors don't help
            "complex", "intricate", "sophisticated",  # Often leads to messy output
            "copyright", "style of", "sounds like",  # Legal/ethical issues
            "minute", "minutes", "seconds"  # Duration in prompt doesn't work
        ]
        ...
        for word in self.avoid_words:
            if word in prompt_lower:
                if word in ["vocal", "vocals", "singing", "voice", "lyrics"]:
                    issues.append(f"Remove '{word}' - MusicGen only generates instrumentals")
                elif word in ["epic", "cinematic", "hollywood"]:
                    issues.append(f"'{word}' is too vague - be more specific about instruments and mood")
                elif word in ["masterpiece", "professional", "high quality"]:
                    issues.append(f"Remove '{word}' - quality descriptors don't improve output")
                else:
                    issues.append(f"Avoid using '{word}'")
        
        # Check length
        if len(prompt) < 10:
            issues.append("Prompt too short - add more detail about style, instruments, and mood")
        elif len(prompt) > 200:
            issues.append("Prompt too long - MusicGen works better with concise descriptions")
        
        # Check for key elements
        has_genre = any(genre in prompt_lower for genre in ["jazz", "rock", "electronic", "classical", "folk", "ambient", "hip hop", "trap"])
        has_instrument = any(inst in prompt_lower for instruments in self.instruments.values() for inst in instruments)
        has_descriptor = any(mod in prompt_lower for modifiers in self.modifiers.values() for mod in modifiers)
    ...
    def improve_prompt(self, prompt: str) -> str:
        ...
        if not any(tempo in prompt_lower for tempo in ["slow", "fast", "moderate", "tempo", "bpm"]):
