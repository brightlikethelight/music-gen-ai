...
from musicgen.vocal_synthesis import VocalSynthesizer, LyricsProcessor, SimpleMelodySynthesizer
...
class TestVocalSynthesizer:
    ...
    def test_apply_singing_effects(self):
        ...
        # Test with vibrato
        result = synth.apply_singing_effects(audio, sr, vibrato_rate=5.0, vibrato_depth=0.1)
        assert len(result) == len(audio)
...
class TestSimpleMelodySynthesizer:
    ...
    def test_melody_patterns(self):
        ...
        try:
            ...
            result = SimpleMelodySynthesizer.apply_melody_contour(audio, sr, "ascending")
...
class TestAudioMixer:
    ...
    def test_resampling(self):
        ...
        mixed, mixed_sr = AudioMixer.mix_tracks(
            instrumental=(inst_audio, 44100),
            vocals=(vocal_audio, 22050)
        )
...
class TestVocalGenPipeline:
    ...
    def test_mix_parameters(self):
        ...
        vocal_level, reverb = pipeline._get_mix_parameters("instrumental_forward", "rock")
...
class TestQuickGenerate:
    ...
    def test_quick_generate_with_vocals(self, mock_pipeline_class):
        ...
        # Mock pipeline
        mock_pipeline = Mock()
        mock_pipeline.generate_with_vocals.return_value = (np.random.randn(44100), 44100)
        mock_pipeline.music_generator = Mock()
        mock_pipeline.music_generator.save_audio = Mock()
        mock_pipeline.music_generator.save_audio_as_format = Mock(return_value="output.mp3")
        mock_pipeline_class.return_value = mock_pipeline
        
        # Test WAV output
        result = quick_generate_with_vocals(
            "pop song",
            "output.wav",
            lyrics="La la la"
        )
