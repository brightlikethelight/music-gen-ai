...
def load_audio(
    file_path: Union[str, Path], target_sr: int = 32000, mono: bool = True
...
def estimate_memory_usage(duration: float, model_size: str = "small") -> dict:
    ...
    model_memory = {"small": 0.5, "medium": 1.5, "large": 3.5}  # 500MB  # 1.5GB  # 3.5GB
...
def crossfade_audio(
    audio1: np.ndarray, audio2: np.ndarray, overlap_seconds: float, sample_rate: int
) -> np.ndarray:
    """
...
def apply_fade(
    audio: np.ndarray, sample_rate: int, fade_in: float = 0.1, fade_out: float = 0.1
) -> np.ndarray:
    """
