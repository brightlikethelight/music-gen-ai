    ...
    def generate_with_progress(self, prompt, duration, **kwargs):
        ...
        with tqdm(total=steps, desc="Generating", unit="steps") as pbar:
            ...
            try:
                ...
                # Generate audio
                audio = self.model.generate_audio(texts=[prompt], duration=duration, **kwargs)
                ...
                logger.info(
                    f"✓ Generated in {generation_time:.1f}s ({duration/generation_time:.1f}x realtime)"
                )
...
def main():
    ...
    parser.add_argument(
        "--prompt", type=str, default="upbeat jazz piano melody", help="Text prompt for generation"
    )
    parser.add_argument("--duration", type=float, default=10.0, help="Duration in seconds")
    parser.add_argument(
        "--output", type=str, default="outputs/test_generation.wav", help="Output file path"
    )
    parser.add_argument(
        "--model-size",
