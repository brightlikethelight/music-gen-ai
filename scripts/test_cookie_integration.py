...
class CookieTestClient:
    ...
    def test_login(self):
        ...
        login_data = {"email": "user@example.com", "password": "password123", "remember_me": False}

        response = self.session.post("/api/auth/login", json=login_data, headers=headers)
...
def main():
    ...
    parser.add_argument("--concurrent", action="store_true", help="Test concurrent sessions")
