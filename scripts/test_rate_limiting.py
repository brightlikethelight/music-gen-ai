...
# Test configurations
TEST_CONFIGS = {
    "direct": {"name": "Direct Connection", "headers": {}, "expected_ip": "client"},
    "nginx": {
        "name": "Behind Nginx",
        "headers": {"X-Real-IP": "203.0.113.42", "X-Forwarded-For": "203.0.113.42, 10.0.0.1"},
    ...
    "forwarded": {
        ...
        "headers": {"Forwarded": 'for="[2001:db8:cafe::17]:4711"; proto=https; by=10.0.0.1'},
        "expected_ip": "2001:db8:cafe::17",
    },
    "internal": {
        "name": "Internal Service",
        "headers": {"X-Internal-Service": "monitoring-service", "X-API-Key": "test-internal-key"},
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(35):  # Free tier: 30/min
                ...
                if i < 30:
                    ...
                    if self.verbose:
                        print(
                            f"Request {i+1}: OK - Remaining: {response.headers.get('X-RateLimit-Minute-Remaining')}"
                        )
                else:
        ...
        async with httpx.AsyncClient() as client:
            for config_name, config in TEST_CONFIGS.items():
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/models", headers=config["headers"]
                )
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(10):  # Free tier burst size: 5
                ...
                burst_results.append(
                    {"request": i + 1, "status": response.status_code, "elapsed": elapsed}
                )
    ...
    async def test_tier_limits(self):
        ...
        async with httpx.AsyncClient() as client:
            for tier in tiers:
                ...
                response = await client.get(
                    f"{self.base_url}/api/v1/models", headers=tier["headers"]
                )
    ...
    async def test_internal_service_bypass(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for i in range(100):  # Should all succeed
                response = await client.get(
                    f"{self.base_url}/api/v1/models", headers=internal_headers
                )
...
def main():
    ...
    if args.test == "all":
        ...
        test_method = getattr(tester, f"test_{args.test}_rate_limiting", None) or getattr(
            tester, f"test_{args.test}", None
        )
        if test_method:
