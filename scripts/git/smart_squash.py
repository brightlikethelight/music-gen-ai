...
def suggest_squash_plan(groups: Dict[str, List[Dict[str, str]]]) -> List[Tuple[str, List[Dict[str, str]]]]:
    """Suggest a squash plan based on commit groups."""
    plan = []
    
    # Define the order we want categories to appear
    category_order = ['initial', 'feature', 'model', 'api', 'ui', 'fix', 'refactor', 'test', 'docs', 'style', 'chore', 'other']
...
def generate_squash_message(category: str, commits: List[Dict[str, str]]) -> str:
    ...
    for commit in commits:
        ...
        cleaned_subject = re.sub(r'^(feat|fix|docs|style|refactor|test|chore|build|ci|perf):\s*', '', cleaned_subject, flags=re.IGNORECASE)
        if cleaned_subject:
