...
class BeamSearcher:
    ...
    def __init__(self, config: BeamSearchConfig):
        ...
        if self.num_beams % self.num_beam_groups != 0:
            raise ValueError(f"num_beams ({self.num_beams}) must be divisible by num_beam_groups ({self.num_beam_groups})")
    ...
    def search(
        ...
        **model_kwargs
        ...
        if conditioning_embeddings is not None:
            conditioning_embeddings = self._expand_for_beams(conditioning_embeddings, self.num_beams)
        ...
        while cur_len < self.max_length:
            ...
            model_inputs = self._prepare_model_inputs(
                ...
                conditioning_embeddings=conditioning_embeddings if cur_len == input_ids.shape[-1] else None,
    ...
    def _expand_for_beams(self, tensor: torch.Tensor, num_beams: int) -> torch.Tensor:
        ...
        expanded_tensor = tensor.unsqueeze(1).expand(
            batch_size, num_beams, *tensor.shape[1:]
        ).contiguous().view(batch_size * num_beams, *tensor.shape[1:])
    ...
    def _beam_search_step(
        ...
        # Extract outputs
        beam_scores_new = torch.tensor([x[0] for x in beam_outputs], device=next_token_scores.device)
    ...
    def _finalize_generation(
        ...
        for batch_idx, hyps in enumerate(generated_hyps):
            sorted_hyps = sorted(hyps, key=lambda x: x.get_length_normalized_score(self.length_penalty), reverse=True)
