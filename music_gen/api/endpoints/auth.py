...
async def login(
    ...
    try:
        ...
        refresh_token = auth_middleware.create_refresh_token(
            user_id=user_id,
            expires_delta=refresh_token_expires // 60
        )
...
async def logout(
    request: Request,
    response: Response,
    user_claims: Optional[UserClaims] = Depends(require_auth)
) -> Dict[str, str]:
    """
...
async def migrate_tokens(
    request: Request,
    response: Response,
    migration_data: MigrateTokenRequest
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        refresh_claims = auth_middleware.verify_token(migration_data.refresh_token)
