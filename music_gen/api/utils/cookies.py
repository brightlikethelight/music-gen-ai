...
class SecureCookieManager:
    ...
    def set_auth_cookies(
        ...
        response.set_cookie(
            key=SecureCookieManager.AUTH_TOKEN_COOKIE, value=access_token, **access_settings
        )
        ...
        response.set_cookie(
            key=SecureCookieManager.REFRESH_TOKEN_COOKIE, value=refresh_token, **refresh_settings
        )
    ...
    def set_csrf_cookie(response: Response, csrf_token: str) -> None:
        ...
        response.set_cookie(key=SecureCookieManager.CSRF_TOKEN_COOKIE, value=csrf_token, **settings)

    @staticmethod
    def set_session_cookie(
        response: Response, session_id: str, expires_in: int = 3600  # 1 hour
    ) -> None:
        """Set session ID cookie."""
        settings = cookie_config.get_cookie_settings(expires_in)
        settings["httponly"] = True

        response.set_cookie(key=SecureCookieManager.SESSION_ID_COOKIE, value=session_id, **settings)
    ...
    def clear_auth_cookies(response: Response) -> None:
        ...
        # Clear access token
        response.set_cookie(key=SecureCookieManager.AUTH_TOKEN_COOKIE, value="", **settings)

        # Clear refresh token (with its restricted path)
        refresh_settings = settings.copy()
        refresh_settings["path"] = "/api/auth/refresh"
        response.set_cookie(
            key=SecureCookieManager.REFRESH_TOKEN_COOKIE, value="", **refresh_settings
        )

        # Clear session cookie
        response.set_cookie(key=SecureCookieManager.SESSION_ID_COOKIE, value="", **settings)

        # Clear CSRF cookie
        response.set_cookie(key=SecureCookieManager.CSRF_TOKEN_COOKIE, value="", **settings)
    ...
    def create_secure_cookie_header(
        name: str, value: str, max_age: Optional[int] = None, **kwargs
    ) -> str:
...
def set_auth_cookies(
    ...
    SecureCookieManager.set_auth_cookies(
        response, access_token, refresh_token, access_expires or 900, refresh_expires or 604800
    )
