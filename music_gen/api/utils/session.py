...
class SessionManager:
    ...
    def create_session(
        ...
        try:
            ...
            duration = (
                self.EXTENDED_SESSION_DURATION if remember_me else self.DEFAULT_SESSION_DURATION
            )
            ...
            self.redis.setex(session_key, duration, session_data.json())
    ...
    def update_session(
        self, session_id: str, data: Dict[str, Any], extend_expiration: bool = False
    ) -> bool:
        """Update session data."""
        if not self.enabled:
            return False
        ...
        try:
            ...
            while True:
                cursor, keys = self.redis.scan(
                    cursor, match=f"{self.USER_SESSIONS_PREFIX}*", count=100
                )
