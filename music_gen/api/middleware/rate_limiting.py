...
RATE_LIMIT_TIERS: Dict[RateLimitTier, RateLimitConfig] = {
    RateLimitTier.FREE: RateLimitConfig(
        requests_per_minute=30, requests_per_hour=500, requests_per_day=5000, burst_size=5
    ),
    RateLimitTier.PREMIUM: RateLimitConfig(
        requests_per_minute=60, requests_per_hour=2000, requests_per_day=20000, burst_size=10
    ),
    RateLimitTier.ENTERPRISE: RateLimitConfig(
        requests_per_minute=300, requests_per_hour=10000, requests_per_day=100000, burst_size=50
    ),
    RateLimitTier.INTERNAL: RateLimitConfig(
        requests_per_minute=1000, requests_per_hour=50000, requests_per_day=1000000, burst_size=100
    ),
...
class RateLimitStorage:
    ...
    def increment(self, key: str, window: int, limit: int) -> Tuple[int, int]:
        """
...
class MemoryRateLimitStorage(RateLimitStorage):
    ...
    def _cleanup_expired(self):
        ...
        current_time = time.time()
        expired_keys = [key for key, (_, expiry) in self.storage.items() if current_time >= expiry]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not allowed:
            ...
            logger.warning(
                f"Rate limit exceeded for {client_id} " f"(tier: {tier}, path: {request.url.path})"
            )
    ...
    def _check_rate_limits(
        ...
        minute_count, minute_ttl = self.storage.increment(
...
def create_rate_limit_middleware(
    ...
    internal_keys = internal_service_keys or set(
        key.strip() for key in os.getenv("INTERNAL_API_KEYS", "").split(",") if key.strip()
    )
