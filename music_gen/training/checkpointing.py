...
class ModelCheckpointManager:
    ...
    def save_checkpoint(
        ...
        filename = (
            f"epoch_{epoch:03d}_step_{step:06d}_{self.monitor}_{monitor_value:.4f}_{timestamp}.ckpt"
        )
...
class ModelSaver:
    ...
    def save_model_for_deployment(
        ...
        if include_tokenizer:
            ...
            tokenizer_config = {
                "model_name": (
                    model.audio_tokenizer.model_name
                    if hasattr(model.audio_tokenizer, "model_name")
                    else "facebook/encodec_24khz"
                ),
                "sample_rate": (
                    model.audio_tokenizer.sample_rate
                    if hasattr(model.audio_tokenizer, "sample_rate")
                    else 24000
                ),
                "num_quantizers": (
                    model.audio_tokenizer.num_quantizers
                    if hasattr(model.audio_tokenizer, "num_quantizers")
                    else 8
                ),
            }
...
class TrainingResumer:
    ...
    def resume_training(
        ...
        # Load checkpoint
        checkpoint_data = self.checkpoint_manager.load_checkpoint(checkpoint_path, model=model)
        ...
        logger.info(
            f"Resumed training from epoch {checkpoint_data.get('epoch', 0)}, "
...
def create_checkpoint_manager(checkpoint_dir: str, **kwargs) -> ModelCheckpointManager:
...
def save_model_for_deployment(
    model: MusicGenModel, save_dir: str, model_name: str = "musicgen_model", **kwargs
) -> str:
