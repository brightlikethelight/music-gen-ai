...
class StreamingState:
    ...
    def update_context(self, new_tokens: torch.Tensor, new_past_key_values: Optional[Tuple]):
        ...
        if len(self.generated_tokens) > self.config.max_context_length:
            ...
            tokens_to_remove = len(self.generated_tokens) - self.config.max_context_length + self.config.context_window_overlap
...
class StreamingGenerator:
    ...
    def __init__(self, model, config: StreamingConfig):
        ...
        logger.info(f"Streaming config: {config.chunk_duration}s chunks = {self.chunk_tokens} tokens, "
    ...
    def _generate_next_chunk(self) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        for step in range(self.chunk_tokens):
            ...
            if past_key_values is None and self.current_state.encoder_outputs:
                model_inputs.update({
                    "encoder_hidden_states": self.current_state.encoder_outputs["text_hidden_states"],
                    "encoder_attention_mask": self.current_state.encoder_outputs["text_attention_mask"],
                    "conditioning_embeddings": self.current_state.encoder_outputs["conditioning_embeddings"],
                })
    ...
    def _apply_generation_params(self, logits: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
    ...
    def _tokens_to_audio_chunk(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        if chunk_frames == 0:
            ...
            return torch.zeros(1, 1, int(self.config.chunk_duration * self.model.audio_tokenizer.sample_rate))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["chunks_generated"] > 0:
            stats["real_time_factor"] = (
                stats["total_duration"] / stats["total_generation_time"]
            )
