...
from .generator import StreamingGenerator, StreamingConfig, create_streaming_generator
from .audio_streamer import AudioStreamer, AdaptiveStreamer, AudioChunk
...
class StreamingSession:
    ...
    def __init__(
        ...
        self.thread_pool = ThreadPoolExecutor(max_workers=2, thread_name_prefix=f"session-{session_id}")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                chunk_data = await asyncio.wait_for(
                    ...
                    timeout=self.generator.config.max_latency_ms / 1000.0
                )
        ...
        try:
            ...
            chunk_id = await asyncio.get_event_loop().run_in_executor(
                ...
                lambda: self.audio_streamer.add_audio_chunk(
                    raw_audio,
                    chunk_data.get("duration", self.request.chunk_duration)
                )
            )
            ...
            audio_segment = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool,
                self.audio_streamer.get_next_audio_segment
            )
    ...
    async def interrupt_and_modify(self, new_prompt: str, **kwargs) -> bool:
        ...
        if self.generator:
            success = await asyncio.get_event_loop().run_in_executor(
                self.thread_pool,
                lambda: self.generator.interrupt_and_modify(new_prompt, **kwargs)
            )
...
class SessionManager:
    ...
    async def _cleanup_inactive_sessions(self):
        ...
        for session_id, session in self.sessions.items():
            ...
            if (
                session.info.state in [SessionState.STOPPED, SessionState.ERROR] or
                current_time - session.info.last_activity > session_timeout
            ):
