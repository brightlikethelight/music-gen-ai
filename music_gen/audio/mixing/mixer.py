...
class MixingEngine:
    ...
    def _init_buses(self):
        ...
        from music_gen.audio.mixing.effects import Reverb, Delay
    ...
    def _process_track(
        ...
        if "volume" in config.automation:
            volume_curve = config.automation["volume"].get_values(output_samples, self.config.sample_rate)
    ...
    def _apply_eq(self, audio: torch.Tensor, config: TrackConfig) -> torch.Tensor:
        ...
        if (config.eq_low_gain == 0 and 
            config.eq_mid_gain == 0 and 
            config.eq_high_gain == 0):
            return audio
        
        eq = EQ(
            sample_rate=self.config.sample_rate,
            bands=[
                {"freq": config.eq_low_freq, "gain": config.eq_low_gain, "q": 0.7, "type": "low_shelf"},
                {"freq": config.eq_mid_freq, "gain": config.eq_mid_gain, "q": 1.0, "type": "bell"},
                {"freq": config.eq_high_freq, "gain": config.eq_high_gain, "q": 0.7, "type": "high_shelf"},
            ]
        )
    ...
    def _apply_panning(self, audio: torch.Tensor, config: TrackConfig) -> torch.Tensor:
        ...
        if "pan" in config.automation:
            pan_curve = config.automation["pan"].get_values(audio.shape[-1], self.config.sample_rate)
    ...
    def bounce_stems(
        ...
        for stem_name, track_names in stem_groups.items():
            ...
            stem_configs = {name: track_configs.get(name, TrackConfig(name=name)) 
