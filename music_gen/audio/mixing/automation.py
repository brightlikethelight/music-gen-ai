...
class AutomationLane:
    ...
    def add_point(self, time: float, value: float, curve_type: InterpolationType = InterpolationType.LINEAR):
        ...
        for i in range(len(self.points) - 1):
            ...
            if mask.any():
                ...
                for j, t in enumerate(segment_times):
                    segment_values[j] = self._interpolate(
                        self.points[i],
                        self.points[i + 1],
                        t
                    )
...
class AutomationPreset:
    ...
    @staticmethod
    def fade_in(duration: float, curve: InterpolationType = InterpolationType.EXPONENTIAL) -> AutomationLane:
    ...
    @staticmethod
    def fade_out(duration: float, curve: InterpolationType = InterpolationType.EXPONENTIAL) -> AutomationLane:
