        ...
        for i in range(num_samples):
            ...
            if level_db > self.threshold - self.knee / 2:
                if level_db < self.threshold + self.knee / 2:
                    ...
                    gain_reduction[:, i] = knee_factor ** 2 * (self.threshold - level_db) * (1 - 1 / self.ratio) / 2
                else:
                    # Above knee
...
class Reverb(BaseEffect):
    ...
    def process(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_allpass):
            allpass_out = self._process_allpass(
                allpass_out,
                self.allpass_buffers[i],
                self.allpass_delays[i]
            )
    ...
    def _init_buffers(self, channels: int, device: torch.device):
        ...
        for delay in self.comb_delays:
            buffer = {
                "data": torch.zeros(delay, device=device),
                "index": 0,
                "filter_state": 0.0
            }
    ...
    def _apply_pre_delay(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(audio.shape[0]):
            ...
            # Update index
            self.pre_delay_buffer["index"] = (self.pre_delay_buffer["index"] + 1) % self.pre_delay_samples
        ...
        if self.tone != 0.5:
            ...
            if self.tone > 0.5:
                ...
                for i in range(1, distorted.shape[1]):
                    filtered[:, i] = alpha * (distorted[:, i] - distorted[:, i-1]) + alpha * filtered[:, i-1]
