...
from music_gen.models.multi_instrument.conditioning import InstrumentConditioner, InstrumentClassifier
...
class MultiInstrumentTransformer(nn.Module):
    ...
    def forward(
        ...
        if self.config.parallel_generation and track_indices is not None:
            ...
            logits = torch.stack(track_logits, dim=1)  # [batch_size, num_tracks, seq_len, vocab_size]
        else:
            # Standard generation
...
class MultiInstrumentMusicGen(MusicGenModel):
        ...
        **kwargs
        ...
        instrument_indices = torch.tensor([
            self.transformer.instrument_conditioner.instrument_embedding.name_to_idx.get(
                inst.lower(),
                len(self.multi_config.get_instrument_names())
            )
    ...
    def _generate_parallel_tracks(
        ...
        all_tokens = torch.zeros(
            batch_size, num_tracks, num_tokens,
