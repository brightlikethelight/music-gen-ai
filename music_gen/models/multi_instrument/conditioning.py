...
from music_gen.models.multi_instrument.config import MultiInstrumentConfig, InstrumentConfig
...
class InstrumentEmbedding(nn.Module):
    ...
    def __init__(self, config: MultiInstrumentConfig):
        ...
        self.instrument_embeddings = nn.Embedding(
            len(self.instrument_names) + 1,  # +1 for unknown/mixed
            config.instrument_embedding_dim
        )
    ...
    def forward(
        ...
        if instrument_names is not None:
            ...
            for name in instrument_names:
                ...
                indices.append(idx)
            instrument_indices = torch.tensor(indices, device=self.instrument_embeddings.weight.device)
...
class InstrumentConditioner(nn.Module):
    ...
    def __init__(self, config: MultiInstrumentConfig):
        ...
        self.projection = nn.Linear(
            config.instrument_embedding_dim + config.hidden_size,
            config.hidden_size
        )
    ...
    def forward(
        ...
        if track_indices is None:
            track_indices = torch.arange(num_tracks, device=device).unsqueeze(0).expand(batch_size, -1)
        ...
        if (self.config.use_instrument_attention and 
            layer_idx is not None and 
            layer_idx in self.config.instrument_cross_attention_layers):
            
            # Expand conditioning to sequence length
            conditioning_expanded = conditioning.unsqueeze(2).expand(-1, -1, seq_len, -1)
            conditioning_flat = conditioning_expanded.reshape(batch_size * num_tracks, seq_len, hidden_size)
        ...
        # Apply conditioning
        hidden_states = self.layer_norm(hidden_states + self.dropout(conditioning.mean(dim=1, keepdim=True)))
