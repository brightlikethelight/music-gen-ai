...
class MultiTrackGenerator:
    ...
    def generate(
        ...
        mixed_audio = self._mix_tracks(processed_tracks, track_configs, mixing_params)
    ...
    def _generate_with_beam_search(
        ...
        return self.model.generate_multi_track(
            prompt=prompt, instruments=instruments, duration=duration, temperature=temperature
        )
    ...
    def _apply_simple_eq(
        self, audio: torch.Tensor, low_gain: float, high_gain: float
    ) -> torch.Tensor:
    ...
    def _apply_simple_reverb(self, audio: torch.Tensor, reverb_amount: float) -> torch.Tensor:
        ...
        for i in range(num_variations):
            ...
            for cfg in base_config:
                varied_cfg = TrackGenerationConfig(
                    ...
                    volume=np.clip(
                        cfg.volume + np.random.uniform(-0.2, 0.2) * variation_strength, 0.1, 1.0
                    ),
                    pan=np.clip(
                        cfg.pan + np.random.uniform(-0.3, 0.3) * variation_strength, -1.0, 1.0
                    ),
                    reverb=np.clip(
                        cfg.reverb + np.random.uniform(-0.1, 0.1) * variation_strength, 0.0, 1.0
                    ),
                    eq_low=np.clip(
                        cfg.eq_low + np.random.uniform(-0.2, 0.2) * variation_strength, -1.0, 1.0
                    ),
                    eq_high=np.clip(
                        cfg.eq_high + np.random.uniform(-0.2, 0.2) * variation_strength, -1.0, 1.0
                    ),
    ...
    def remix_tracks(
        ...
        return self._mix_tracks(existing_tracks, new_mixing_configs, dummy_mixing_params)
