...
class ConditioningEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Layer norm will be applied dynamically based on actual output size
    ...
    def forward(
        ...
        # Apply layer norm based on actual tensor size
        conditioning = F.layer_norm(conditioning, conditioning.shape[-1:])
        ...
        if output_projection_dim is not None:
            if self.text_encoder.hidden_size != output_projection_dim:
                self.text_projection = nn.Linear(
                    self.text_encoder.hidden_size, output_projection_dim
                )

            if self.conditioning_encoder.output_dim != output_projection_dim:
                self.conditioning_projection = nn.Linear(
                    self.conditioning_encoder.output_dim, output_projection_dim
                )
